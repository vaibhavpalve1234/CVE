...
Run from the repository root folder (`CVE/`).

```bash
# 1) Build/refresh index first
python scripts/build_index.py
...
If you prefer module mode, these are equivalent:

```bash
python -m src.cli --query "Show critical remote code execution CVEs" --top-k 10
```
